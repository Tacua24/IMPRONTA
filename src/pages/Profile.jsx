...
export default function Profile() {
  const { user, register, login, logout, upsertArtistProfile, artistProfileOf, loading } = useAuth();
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (mode === "register") {
        ...
        const effectiveRole = (form.role || registered?.role || "").toLowerCase();
        if (effectiveRole === "artist") {
          await upsertArtistProfile(registered.id, artist);
        }
      } else {
  ...
  if (loading) {
    return (
      <main className="min-h-[70vh] flex items-center justify-center px-6 sm:px-10 py-16">
        <p className="text-sm text-neutral-600">Cargando tu sesión…</p>
      </main>
    );
  }
  ...
  const ap = user.role === "artist" ? artistProfileOf(user.id) : null;
      ...
      <div className="grid gap-6 max-w-4xl">
        <div className="bg-white/70 border border-black/10 rounded-xl p-6">
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <dd className="font-medium">{user.full_name || user.name || "—"}</dd>
            </div>
            <div>
          ...
          <div className="mt-6">
            <button onClick={logout} className="px-3 py-2 rounded bg-black text-white text-sm">
              Salir
            </button>
          </div>
        </div>
        ...
        {ap && (
          <div className="bg-white/70 border border-black/10 rounded-xl p-6">
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div>
                <dt className="text-neutral-500">Nombre artístico</dt>
                <dd className="font-medium">{ap.stage_name || "—"}</dd>
              </div>
              <div>
                <dt className="text-neutral-500">Ubicación</dt>
                <dd className="font-medium">{[ap.city, ap.country].filter(Boolean).join(", ") || "—"}</dd>
              </div>
              <div>
                <dt className="text-neutral-500">Años</dt>
                <dd className="font-medium">
                  {ap.birth_year || "—"}
                  {ap.death_year ? `– ${ap.death_year}` : ""}
                </dd>
              </div>
              <div>
                <dt className="text-neutral-500">Instagram</dt>
                <dd className="font-medium">{ap.instagram || "—"}</dd>
              </div>
              <div className="sm:col-span-2">
                <dt className="text-neutral-500">Bio</dt>
                <dd className="font-medium">{ap.bio || "—"}</dd>
              </div>
            </dl>
          </div>
        )}
