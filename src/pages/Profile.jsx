...
export default function Profile() {
    ...
    const [artist, setArtist] = useState({
     ...
     const onSubmit = async (e) => {
        ...
        if ((form.rol || registered.role) === "artista") {
          upsertArtistProfile(registered.id, artist); // guarda perfil 1:1 (mock de tabla artists)
                }
            } else {
                ...
                <div className="grid gap-6 max-w-4xl">
                    <div className="bg-white/70 border border-black/10 rounded-xl p-6">
                        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div><dt className="text-neutral-500">Nombre</dt><dd className="font-medium">{user.full_name || user.name}</dd></div>
